...
import { getSubmission, getAuditLog, getSubmissions } from "@/lib/db";
import { decideAction, saveNotesAction } from "@/lib/actions";
...
import type { Submission, AuditEntry, ScoreFactor, PremiumModel } from "@/lib/types";
  ...
  return (
    <div className="group relative">
      ...
      <div className="flex items-center gap-2">
        {f.impact <= 0 && (
          <div className="flex-1 h-1.5 rounded-full overflow-hidden flex justify-end" style={{ background: "rgba(255,255,255,0.04)" }}>
...
function PremiumRange({ model }: { model: PremiumModel }) {
  const fmt    = (n: number) => `£${n.toLocaleString("en-GB")}`;
  const range  = model.high - model.low;
  const midPct = range > 0 ? ((model.mid - model.low) / range) * 100 : 50;
  return (
    <div>
      ...
      <div className="relative h-2 rounded-full mb-2" style={{ background: "rgba(255,255,255,0.06)" }}>
        <div className="absolute top-1/2 -translate-y-1/2 w-3 h-3 rounded-full border-2 border-white shadow-lg"
          style={{ left: `${midPct}%`, background: "var(--brand)" }} />
      </div>
...
export default function SubmissionDetail() {
  ...
  const decide = useCallback(async (decision: "accepted" | "declined") => {
    ...
    setStatus(decision);
    try {
      await decideAction(id, decision, "Max", note || undefined);
      toast.success(decision === "accepted" ? "Submission accepted" : "Submission declined", {
        description: `${id} · ${sub?.extracted_data?.insured_name ?? ""}`,
      });
      await load();
    } catch {
      toast.error("Failed to save decision");
      setStatus(sub?.status ?? "referred");
    } finally {
      setDeciding(false);
    }
  }, [id, deciding, note, sub, load]);

  const saveNotes = async () => {
    setSaving(true);
    try {
      await saveNotesAction(id, note);
  ...
  const exportCDR = () => toast.success("Lloyd's CDR export ready", { description: "ACORD 28 formatted data copied to clipboard" });
  ...
  if (!sub) {
    return (
      <div className="p-6 text-center py-24">
        <p className="text-slate-500 mb-4">Submission {id} not found.</p>
        <Link href="/dashboard/submissions" className="text-sm text-indigo-400 hover:text-indigo-300">
          ← Back to submissions
        </Link>
      </div>
    );
  ...
  const comparables = allSubs
    .filter(s => s.id !== sub.id && s.extracted_data?.coverage_type === ex?.coverage_type && s.score !== null)
    .slice(0, 3);
  const brokerHistory = allSubs
    .filter(s => s.broker_company === sub.broker_company && s.id !== sub.id)
    .slice(0, 4);
  const brokerSubs    = allSubs.filter(s => s.broker_company === sub.broker_company && s.status !== "processing");
  const brokerBindRate = brokerSubs.length
    ? Math.round((brokerSubs.filter(s => s.status === "accepted").length / brokerSubs.length) * 100)
    : 0;
  const brokerAvgScore = brokerSubs.filter(s => s.score).length
    ? Math.round(brokerSubs.filter(s => s.score).reduce((a, s) => a + (s.score ?? 0), 0) / brokerSubs.filter(s => s.score).length)
    : 0;
  ...
  const scoreFactors   = ex?.score_factors ?? [];
  const maxImpact      = scoreFactors.length ? Math.max(...scoreFactors.map(f => Math.abs(f.impact))) : 30;
  const totalPositive  = scoreFactors.filter(f => f.impact > 0).reduce((a, f) => a + f.impact, 0);
  const totalNegative  = scoreFactors.filter(f => f.impact < 0).reduce((a, f) => a + f.impact, 0);
      ...
      <div className="flex items-start justify-between mb-5">
        <div>
          <div className="flex items-center gap-2 mb-1.5">
            ...
            <StatusBadge status={status as "pending" | "processing" | "accepted" | "declined" | "referred"} />
            {sub.file_name && (
              <>
                <span className="text-slate-700">·</span>
          ...
          <div className="flex items-center gap-3 mt-1 flex-wrap">
            {ex?.coverage_type && <span className="text-sm text-slate-400">{ex.coverage_type}</span>}
            ...
            <>
                ...
                <div className="card p-4">
                  ...
                  {sub.score !== null ? (
                    <>
                      ...
                      {riskLevel && (
                        <span className="inline-flex px-2 py-0.5 rounded text-xs font-semibold" style={{ background: riskLevel.bg, color: riskLevel.color }}>
                          {riskLevel.label}
                        </span>
                      )}
                      {ex?.confidence_score !== undefined && (
                        <p className="text-[10px] text-slate-700 mt-2">AI confidence: {Math.round(ex.confidence_score * 100)}%</p>
                      )}
                    </>
                  ) : (
                ...
                <div className="col-span-2 card p-4 grid grid-cols-3 gap-3">
                  {ex ? [
                    ...
                    <div className="col-span-3 flex items-center gap-2 text-slate-600 text-sm">
                      <span className="w-2 h-2 rounded-full bg-blue-400 animate-pulse" /> Extracting…
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="card p-4">
                ...
                <button onClick={saveNotes} disabled={saving}
                  className="mt-2 text-xs px-3 py-1.5 rounded-lg text-slate-400 hover:text-white transition-colors"
                    ...
                    {auditLog.map((entry, i) => (
                      <div key={entry.id} className="flex gap-3">
                        <div className="flex flex-col items-center">
                          ...
                          {i !== auditLog.length - 1 && <div className="w-px flex-1 mt-1" style={{ background: "var(--border)" }} />}
                        </div>
              ...
              {scoreFactors.length > 0 ? (
                <div className="card p-5">
                  ...
                  <p className="text-xs text-slate-600 mb-5">
                    Factor contributions to the final risk score of <strong className="text-white">{sub.score}</strong>/100
                  </p>
                  <div className="flex items-center gap-3 mb-5 text-xs text-slate-500">
                    <span className="flex items-center gap-1"><TrendingDown size={11} className="text-red-400" /> Negative (left)</span>
                    <div className="w-px h-4" style={{ background: "var(--border)" }} />
                    <span className="flex items-center gap-1"><TrendingUp size={11} className="text-emerald-400" /> Positive (right)</span>
                  </div>
                  <div className="space-y-3">
                    {scoreFactors.map(f => <ScoreFactorBar key={f.label} f={f} max={maxImpact} />)}
                  </div>
                  <div className="flex items-center justify-between mt-5 pt-4" style={{ borderTop: "1px solid var(--border)" }}>
                    <div className="flex items-center gap-2">
                      <div className="px-2 py-1 rounded text-xs font-semibold" style={{ background: "rgba(16,185,129,0.1)", color: "#10b981" }}>+{totalPositive}</div>
                      <div className="px-2 py-1 rounded text-xs font-semibold" style={{ background: "rgba(239,68,68,0.1)", color: "#ef4444" }}>{totalNegative}</div>
                    </div>
                    <span className="text-xs text-slate-600">Model: claude-sonnet-4-6 · {Math.round((ex?.confidence_score ?? 0) * 100)}% confidence</span>
                  </div>
                </div>
              ) : (
                <div className="card p-5 text-center py-12">
                  <Info size={20} className="text-slate-700 mx-auto mb-2" />
                  <p className="text-sm text-slate-500">AI factor breakdown not available for this submission.</p>
                  <p className="text-xs text-slate-600 mt-1">Submissions processed after the v3 upgrade include full explainability data.</p>
                </div>
              )}
              ...
              {comparables.length > 0 && (
                <div className="card p-5">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <span className="text-sm font-semibold text-white">Comparable risks</span>
              ...
              <div className="card p-4">
                ...
                <div className="grid grid-cols-2 gap-3 text-xs">
                  {[
                    ...
                    { label: "Confidence",         value: `${Math.round((ex?.confidence_score ?? 0) * 100)}%` },
                    { label: "Factors evaluated", value: scoreFactors.length ? String(scoreFactors.length) : "N/A" },
                    { label: "EU AI Act class",   value: "High-risk (Art. 6)" },
                    { label: "Human oversight",   value: "Required at 50–70 threshold" },
                  ].map(r => (
            ...
            <>
              {ex?.premium_model ? (
                <>
                  ...
                  {scoreFactors.length > 0 && (
                    <div className="card p-4">
                      ...
                      <div className="space-y-2">
                        {scoreFactors.map(f => {
                          const adj = Math.round((f.impact / 100) * ex.premium_model!.mid * -0.8);
                          return (
                ...
                <div className="card p-5 text-center py-12">
                  ...
                  <p className="text-sm text-slate-500">Premium model not available for this submission.</p>
                </div>
              )}
              <div className="card p-4 text-xs text-slate-600 leading-relaxed">
                <p className="font-medium text-slate-500 mb-1">Disclaimer</p>
                Indicative premium model generated by Velox AI. Not a binding quote. Subject to underwriter review and final terms agreed with the insured. All figures in GBP exclusive of IPT (12%).
              </div>
            </>
          )}
        </div>
        ...
        <div className="space-y-4">
          ...
          {(status === "referred" || status === "pending") ? (
            <div className="card p-4">
              ...
              <div className="space-y-2">
                <button onClick={() => decide("accepted")} disabled={deciding}
                  ...
                  <kbd className="ml-auto text-[10px] font-mono text-slate-600">A</kbd>
                </button>
                <button onClick={() => decide("declined")} disabled={deciding}
                  ...
                  <kbd className="ml-auto text-[10px] font-mono text-slate-600">D</kbd>
                </button>
              </div>
            </div>
          ) : (
            <div className="card p-4">
              ...
              {sub.decision_at && (
                <p className="text-xs text-slate-700 mt-1">
                  {formatDistanceToNow(parseISO(sub.decision_at), { addSuffix: true })}
                </p>
              )}
            </div>
          )}
          ...
          <div className="card p-4">
            ...
            <div className="grid grid-cols-3 gap-2 mb-3">
              {[
                { label: "Submissions", value: allSubs.filter(s => s.broker_company === sub.broker_company).length },
                { label: "Bind rate",   value: `${brokerBindRate}%` },
                { label: "Avg score",   value: brokerAvgScore || "—" },
              ].map(r => (
          ...
          {/* CDR */}
          <button onClick={exportCDR} className="w-full card p-4 text-left group hover:border-indigo-500/30 transition-colors">
            ...
            <p className="text-[11px] text-slate-600 leading-relaxed">
              ACORD 28 format for the Lloyd&apos;s Central Data Repository. Blueprint Two compliant.
            </p>
